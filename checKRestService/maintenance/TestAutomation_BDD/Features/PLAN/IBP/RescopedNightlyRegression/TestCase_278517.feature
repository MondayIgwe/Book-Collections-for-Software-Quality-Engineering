...
Scenario: WEB_TPM_IBP_APPLYVALUE_005
	...
	# Step 4 - Open filter on customer "F_MEASURE3" @Filter1,"F_MEASURE4" @Filter2,  Deselect all the choises, Select @Customer,v
	When the user changes the filter on Filter Name: '[F_MEASURE3]' to Filter Value: '<CustomerFilters>' and filters with User Code: 'User Code', Filter Type: 'Like' in the IBP Shell Tab
	And the user changes the filter on Filter Name: '[F_MEASURE4]' to Filter Value: '<MonthFilters>' and filters with User Code: 'User Code', Filter Type: 'Like' in the IBP Shell Tab
