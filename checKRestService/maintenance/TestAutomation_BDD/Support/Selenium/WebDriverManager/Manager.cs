...
using Kantar_BDD.Support.Selenium.WebDriverManager.Models;

namespace Kantar_BDD.Support.Selenium.WebDriverManager
{
    /// <summary>
    ...
    public class Manager
    {
       ...
       /// </summary>
        ...
        /// <returns></returns>
        internal static Browser GetBrowser(BrowserType browserType)
        {
            return GetBrowsers().FirstOrDefault(x => x.Name.Replace(" ", "_").ToUpper().Contains(browserType.ToString()));
        }
        ...
        internal static T Get<T>(string endpoint)
        {
            ...
            var response = httpClient.ExecuteGetAsync(request);
         ...
         */
        internal static string GetFireFoxURL()
        {
            try
            {
                ...
                {
                    var asset = responseData.Assets.FirstOrDefault(x => x.Name.ToLower().Contains("win32"));
                    return asset.browser_download_url.AbsoluteUri;
                }
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                {
                    var asset = responseData.Assets.FirstOrDefault(x => x.Name.ToLower().Contains("macos"));
                    return asset.browser_download_url.AbsoluteUri;
                }
                else
                {
                    var asset = responseData.Assets.FirstOrDefault(x => x.Name.ToLower().Contains("linux" + (sixtyfourBit ? "64" : "32")));
                    return asset.browser_download_url.AbsoluteUri;
                }
            }
            catch (Exception e)
        ...
        internal static string GetVersionFromGoogleAPIS(string url, Browser browser)
        {
            ...
            List<Contents> version = !browser.Name.Equals("Google Chrome")
                ? data.Contents.Where(x => x.Key.Contains(browser.Version.Split(".")[0])).ToList()
                : data.Contents.Where(x => x.Key.Split(".")[0].Contains(browser.Version.Split(".")[0])).ToList();
            bool sixtyfourBit = Environment.Is64BitOperatingSystem;
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                return version.FirstOrDefault(x => x.Key.ToLower().Contains("win32")).Key;
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {

                return version.FirstOrDefault(x => x.Key.ToLower().Contains("mac" + (sixtyfourBit ? "64" : "32"))).Key;
            }
            else
            {
                return version.FirstOrDefault(x => x.Key.ToLower().Contains("linux" + (sixtyfourBit ? "64" : "32"))).Key;
            }
        }
        ...
        private static string DownloadAndUnzip(string url, Browser browser, string filename)
        {
            ...
            using (var client = new WebClient())
            {
                ...
                if (File.Exists(destination + "msedgedriver.exe") && !File.Exists(destination + "MicrosoftWebDriver.exe"))
                {
