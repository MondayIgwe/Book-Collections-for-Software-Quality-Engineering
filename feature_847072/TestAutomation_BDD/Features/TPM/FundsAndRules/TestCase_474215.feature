...
@BDD_474215 @PROMO_Nightly @FundsAndRules @FundsAndRulesNightly
Scenario: WEB_TPM_SP_FRDOCUMENT_RATEBASEDFUND_NEGC_MOV_001
	...
	And user adds a ConstraintField: '% Rate Fund' constraint with value ConstraintValue: '<INITIALRATEFUND>' startDate: '<START_DATE>' endDate: '<END_DATE>' familyConstraints: ''
	...
	And the user validates that the 'Text' with value 'POSC' is not present
	And the user validates that the 'Text' with value 'NEGC' is not present
