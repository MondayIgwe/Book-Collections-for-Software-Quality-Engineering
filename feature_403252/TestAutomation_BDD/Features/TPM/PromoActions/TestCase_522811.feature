...
@BDD_522811 @PROMO_Nightly @PromoActions
Scenario: WEB_TPM_SP_PRODUCTGRID_QTYESTIMATED_CALCULATION_SKU_AND_DISPLAYS_001
	...
	When the user creates a Promo '<CUSTLEVEL>' '<Customer_Holder_P1>'
	And the user populates the cockpit '<Description_P1>' '<SELL_IN_START_P1>' '<SELL_IN_END_P1>' '<SELL_OUT_START_P1>' '<SELL_OUT_END_P1>' '<Mechanic_P1>'
	#3 From Products tab add a @Display product @DisplayProd , @DisplayProd_Descr
	And the user adds a product '<Display>' '<DisplayProd>'
	#4 From Products tab add 2 SKU's @Product @SKUProd1 @SKUProd2 
	And the user adds a product '<Product>' '<SKUProd1>'
	And the user adds a product '<Product>' '<SKUProd2>'
