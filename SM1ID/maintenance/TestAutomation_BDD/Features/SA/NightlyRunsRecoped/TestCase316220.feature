...
Scenario: WEB_SA_CLAIM_CLAIM DOCUMENT_004
    ...
    # Step 3 - 5
    When the user creates a New Claim with Document Type: 'DocumentType1' Document Number: '<DocumentNumber>' Level: '<Level>' Customer: '<Customer>' Amount: '<Amount>'
    Then the user validates that the 'Add Button' is displayed
    And the user validates the 'Edit Button' is enabled
