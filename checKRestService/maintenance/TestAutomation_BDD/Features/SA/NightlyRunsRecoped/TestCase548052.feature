...
Scenario: TC548052 - WEB_SA_AGREEMENT_AGREEMENT DOCUMENT_CONDITIONS_076
    ...
    Then the user validates that the 'Popup' with value 'Filter Maker' is displayed
    When the user clicks on 'Popup Ok Button' with value 'Filter Maker'
	And the user enters 'AGREE DOC_COND_076_HierFILTER' in field 'Input By Label Name' with value 'Filter Name'
	And the user moves the focus away from the element 
    And the user clicks on 'Popup Ok Button' with value 'Save Filter (Required)'
	...
	And the user validates that popup with sm1 ID 'cardRebateProducts' and name 'Rebate Products' column 'Value' has '10 / 3' with row text '<Status>'
	...
	Then the user validates that the Loading Mask is not present
    And the user validates that the 'Element by SM1Id' with value 'CHKFLGSAMEFILTER' is displayed
	...
	And the user clicks on the 'Close Tab'
	Then the user validates that the Loading Mask is not present
    And the user validates that the 'Element by SM1Id' with value 'ACTION_edit' is displayed

	# Step 16
	When the user clicks on 'RebateCode' text on the grid
	Then the user validates that the Loading Mask is not present
    And the user validates that the 'Element Background Color' with value '008FE1' is displayed
	And the user validates that popup with sm1 ID 'cardRebatesHeader' and name 'Rebate Header' column 'Target' has '<Target1>' with row text '<Description>'
	...
	| Level 4       | EC86217  | Supply         | Year End Bonuses | Brackets   | Net sales revenue | Invoice payable | Monthly          | 50.00      | 50.00      | 2.00     | 75.00      | 10.00      | 3.00     | Brand     | In       | 01    | Level 1   | EC87135      | 909090         | Not aggregated | ALL        | ea7400           | 2       | Draft  | 1.00       | 008fe1         | Aggregated | All Products | Details      | AGREE DOC_COND_076_REBATE |