...
Scenario: Filter on value not visible, not quick, for ROL-MEAS aggregated grid
	...
	When the user changes the filter on Filter Name: '<FilterCust>' to Filter Value: '<FilterCustValue>' and filters with User Code: 'User Code', Filter Type: 'Like' in the IBP Shell Tab

	# Step 7 - Open the @FilterProd
	# Step 8 - Deselect all options
	# Step 9 - Select @FilterProdValue
	# Step 10 - Press on the "OK" button
	When the user changes the filter on Filter Name: '<FilterProd>' to Filter Value: '<FilterProductValue>' and filters with User Code: 'User Code', Filter Type: 'Like' in the IBP Shell Tab
