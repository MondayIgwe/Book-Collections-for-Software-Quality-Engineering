...
@SFA_Nightly @BDD_641932 @SFA_Weekly
Scenario: WEB_SFA_SF_BUDGET_009_CREATE_OPEN_RULE_FOR_FREE_GOODS_BUDGET
	...
	And the user selects validity period from: 'd+5/m/y' to: '27/12/2099' on the 'Generic Drop Down Trigger' element with value 'Valid Period'
	...
	Then the user validates that the 'Text in Grid' with value 'RuleCode' is visible
	...
	And the user validates that the 'Text in Grid Containing' with date value 'd+5/m/y' is visible
	And the user validates that the 'Text in Grid Containing' with date value '27/12/2099' is visible
	...
	And the user validates that the 'Navigation Error Icon' with value 'Cannot edit Advanced Pricing Action because it was opened from a read-only budget' is visible
	And the user validates that the 'Button' with value 'Save' is not displayed
	And the user validates that 'Input By Label Name' with value 'Description' is read-only
