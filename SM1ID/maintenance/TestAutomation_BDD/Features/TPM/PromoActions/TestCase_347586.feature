...
@BDD_347586 @PROMO_Nightly @PromoActions @PromoActionsNightly
Scenario: TPM_SP_W_SMOKE_R80V1_0018 - Re-Confirm Native What-If Scenario in Draft Promo
	...
	And the user populates the cockpit with description: '<PROMODESC1>' sellInStart: '<SELLINSTART>' sellInEnd: '<SELLINEND>' sellOutStart: '<SELLOUTSTART>' sellOutEnd: '<SELLOUTEND>' mechanics: '<MECH>'
		And the user adds a product with productLevel: '<PRODLVL>' productCodes: '<PRODCODE>'
	...
	And the user creates a new Promotional What If with description 'version1'
	...
	When the user changes the Promo What If with description 'version1'
	#Check that the values are different respect to the ones shown before
