...
@BDD_458045 @PROMO_Nightly @PromoPlan
Scenario: WEB_TPM_SP_PROMOPLANDOCUMENT_DRAGDROP_CREATENEW_PROMO_001
	...
	And the user populates the cockpit '<DESCR_NATIONAL>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECH>'
	And the user adds a product '<PRODLEVEL>' '<PRODCODE>'
	And the user adds a product '<PRODLEVEL>' '<PRODCODE2>'
	...
	Then the user validates that 'Input By Label Name' with value 'Customer holder' contains text '<PPHOLDER>'
	...
	And the user adds products '<PRODLEVEL>' '<PRODCODE>'
	And the user adds products '<PRODLEVEL>' '<PRODCODE2>'
	And the user clicks on the 'Save Button'
	And the user clicks on the 'Close Tab'
	#The promotion is present in the promo plan calendar
	Then the user validates that the 'Calendar Promo Plan' with value '<DESCR_NATIONAL2>' is present
