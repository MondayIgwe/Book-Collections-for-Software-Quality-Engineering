...
Scenario: WEB_TPM_SP_PROMOACTION_PREVENT_EDIT_NONRESPONSIBLE_USER_US592949_AC2_ON_004
	...
	And the user select value '<PRODLEVEL>' from the combo 'Product Level Combo'
	...
	And the user clicks on the 'Product Segment Select Trigger'
	...
	#And the user select value '<SCENDESC2>' from the combo 'Active Scenario Combo'
	And the user select value containing '<SCENDESC2>' from the combo 'Active Scenario Combo'
	...
	And the user clicks on the Action 'Scenario ReAssign Action Link'
