...
Scenario: TC_731958_WEB_SFA_SF_ORDER_042_ANOMALIES_AND_MOVEMENTS_UPDATE_ON_SAVE_AND_CONFIRM_BDD
	...
	Then the user changes the Sales Order Customer Reponsibility User: '<FirstAvailableUser>'
	...
	Then 'Toolbar Status Field' is populated with '<Status1>'
	...
	And the user clicks on 'Top Tab' with value 'General Info'
	When the user retrieves the 'Order Number Field' text as 'Order Number'
	And the user closes all pages without saving the work done where there is no pop up
	
	#Step 3
	When the user filters the column by column name 'Order Number', 'Like', 'Order Number'
	And the user clicks on 'Select Indexed Row' with value '1'
	And the user opens the link menu option:'Budget Movements'
	And the user filters the column by column name 'Operation Type', 'Like', '<OperationType>'
	Then 'Toolbar Grid Record Count' is populated with '0/0'
	When the user closes all pages without saving the work done where there is no pop up
	
	#Step 4
	Then the user is on the 'Sale Orders' page
	...
	And the user is on the 'Anomaly Report Tab' page
	Then 'Toolbar Status Field' is populated with '<Status1>'
	When the user filters the column by column name 'Description', 'Like', '<AnomalyReportDescription>'
	And the user clicks on the 'Save Button'
	
	#Step 5
	And the user clicks on the 'Confirm Order Button'
	Then 'Toolbar Status Field' is populated with '<Status2>'
	And the user closes all pages without saving the work done where there is no pop up
