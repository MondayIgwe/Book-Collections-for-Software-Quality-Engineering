...
namespace Kantar_BDD.Support.Selenium
{
    public class SeleniumFunctions
    {
        ...
        /// Gets text from a WebElement
        /// </summary>
        /// <param name="by">Abstracted by locator for the element to retrieve text from</param>
        /// <returns>String text retrived from the element</returns>
        public string GetText(AbstractedBy by)
        {
            ...
            while (count < 3)
            {
                ...
                {
                    count++;
                    var element = GetVisibleElement(by);
                    ScrollUsingCoordinates(element);
                    if (element.TagName.Equals("input") || element.TagName.Equals("textarea"))
                    {
                        return element.GetAttribute("value");
        ...
        public bool ClickUntilElementIsDisplayed(AbstractedBy elementToClick, AbstractedBy elementToValidate, int maxTries = 2)
        {
            int count = 0;
            while (!ValidateElementDisplayed(elementToValidate, 5) && count < maxTries)
            {
                count++;
        ...
        /// <summary>
        /// Waits for the WebElement(s) to load 
        /// </summary>
        ...
        public void FluentWaitForText(AbstractedBy by, string expectedText, int timeout = 15, bool strict = true)
        {
            ...
            Func<IWebElement, bool> waitResult = new Func<IWebElement, bool>((IWebElement ele) =>
            {
                ...
                if (elements.Count <= 1)
                {
                    ...
                    {
                        return GetText(by, timeout).Trim().Contains(expectedText.Trim());
                    }
                ...
                else
                {
                    ...
                    {
                        return GetText(elements.Where(drv => drv.Enabled && drv.Displayed).FirstOrDefault()).Trim().Contains(expectedText.Trim());
                    }
                }
            }
            );
        ...
        /// </summary>
        /// <param name="by">AbstractedBy locator for the  element(s)</param>
        public void ValidateAllElementsLoaded(AbstractedBy By, int loopTimeOut = 15)
        {
            int count = 0;
            int positiveIterations = 0;
