...
@BDD_573256 @PROMO_Nightly @noSellOutDate @PromoPlan
Scenario: WEB_TPM_SP_PROMODOCUMENT_TOOLBAR_PROMOPLANSCENARIOS_001
	...
	And the user clicks on the Promo Action 'Update Description'
	#5 Set in the popup: Selected Scenario = @SOURCESCENARIOID = 1  Description = @BASESCENARIODESC  = "Base Scenario"
	And the user select value '<SOURCESCENARIOID>' from dropdown list 'Selected Scenario'
	And the user enter text '<BASESCENARIODESC>' on the field with label 'Scenario Description'
	And the user clicks on 'Popup Ok Button' with value 'Update Scenario Description'
	...
	And the user creates a Promo '<CUSTLEVEL>' '<CUSTOMER>'
	And the user populates the cockpit '<DESCPROMO>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECH>'
	And the user adds a product '<PRODLVL>' '<PRODCODE1>'
	And the user adds a product '<PRODLVL>' '<PRODCODE2>'
	...
	Then the user validates that product '<PRODNAME1>' is under tier 'National'
	And the user validates that product '<PRODNAME2>' is under tier 'National'
	...
	And the user clicks on 'Popup Ok Button' with value 'Scenarios Selection'
