...
@BDD_457489 @PROMO_Nightly @FundsAndRules
Scenario: WEB_TPM_SP_FRDOCUMENT_REMOVECONSTRAINTS_003
	...
	When the user creates a new fund and rules '<CONFIGURATIONID>' '<CUSTOMERLEVEL>' '<DESCRIPTION>' '<STARTSELLIN>' '<ENDSELLIN>' '<PRODUCT_GROUPS>' '<RATEFUNDSOURCE>'
	#2 Add 2 Constraints:
	And the user clicks on 'Side Panel Tab' with value 'Constraints'
	And user adds a 'Fund value' constraint with value '<VALUE>' '<VALIDITYSTART1>' '<VALIDITYEND1>' ''
	And the user clicks on 'Side Panel Tab' with value 'Summary'
	And the user clicks on 'Side Panel Tab' with value 'Constraints'
	And user adds a 'Fund value' constraint with value '<VALUE2>' '<VALIDITYSTART2>' '<VALIDITYEND2>' '<FAMILYCONSTRAINTS>'
	...
	And the user populates the cockpit '<DESCRIPTION>' '<PROMOSELLINSTART>' '<PROMOSELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECHANIC>'
	...
	And the user filters the column by column name 'Funds & Rules code', 'Like', 'FundsRulesCode'
	...
	And the user clicks on 'Side Panel Tab' with value 'Constraints'
	And the user clicks on '<VALIDITYEND2>' text on the grid
	...
	When the user clicks on '<VALIDITYEND1>' text on the grid
	And the user clicks on the 'Remove Button'
	...
	Examples:
		...
		| Sales Fund      | 10/01/y     | 01/12/y   | WEB_TPM_SP_FRDOCUMENT_REMOVECONSTRAINTS_003 | Level 5       | 10/01/y        | 30/06/y      | 1000  | 2000   | 01/07/y        | 01/12/y      | 01,02               | Confirm | Modified | EC86170  | 01/10/y          | 12/20/y        | Leaflet  | Product | 009     | 100           | To be authorized | Confirm         |                |                |              |            |