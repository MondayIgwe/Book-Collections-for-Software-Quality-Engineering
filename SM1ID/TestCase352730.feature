...
Scenario: WEB_SA_BALANCE_BALANCE_DOCUMENT_ADJUSTMENT_003_Finalization_step
	...
	When the user creates a Promo with customerHLevel: '<level>' customerCode: '<customer>'
	And the user populates the cockpit with description: '<promodescription1>' sellInStart: '<SELLINSTART>' sellInEnd: '<SELLINEND>' sellOutStart: '<SELLOUTSTARTDATE>' sellOutEnd: '<SELLOUTENDDATE>' mechanics: '<mech1>'
	And the user clicks on the 'General Info Tab'
	And the user clicks on the 'Header Info Top Tab' 
    When the user retrieves text from 'Action Code Textbox' and saves it as 'ActionCode1'
	...
	When the user creates a Promo with customerHLevel: '<level>' customerCode: '<customer>'
	And the user populates the cockpit with description: '<promodescription2>' sellInStart: '<SELLINSTART>' sellInEnd: '<SELLINEND>' sellOutStart: '<SELLOUTSTARTDATE>' sellOutEnd: '<SELLOUTENDDATE>' mechanics: '<mech1>'
	And the user clicks on the 'General Info Tab'
	And the user clicks on the 'Header Info Top Tab' 
    When the user retrieves text from 'Action Code Textbox' and saves it as 'ActionCode2'
    ...
    When the user retrieves text from 'Document ID Textbox' and saves it as 'ClaimDocumentID'
	And the user clicks on the 'Matchings Tab'
	And the user selects action 'Match To Generic Amounts' and clicks: 'Match To Payable Amounts'
	...
	And user creates a new balance with Customer Level: '<level>' Customer: '<customer>' Balance Year: '<BalanceYear>' Supplier: '<Supplier>'
	#7 Click on Selector button
	#8 Click on Promo Selector
	And the user selects action 'Selector' and clicks: 'Promo Selector'
	...
	Then user validates that 'AbsoluteAmountToBalance' is a result of value 1: 'ABSOPA1' operator: 'percent' value 2: 'BalancePercentage1'
	...
	Then user validates that 'AbsoluteAmountToBalance2' is a result of value 1: 'ABSOPA2' operator: 'percent' value 2: 'BalancePercentage2'
	...
	Then user validates that 'TotalBalanceAmount' is a result of value 1: 'AmountToBalance1' operator: 'plus' value 2: 'AmountToBalance2'
	...
	Then the user validates the 'Cancel Button' is enabled
	#15 Click on Cancel or Close the popup (X)
	When the user clicks on the 'Total Balance Amount Textbox'
	...
	#7 Click on Selector button
	#8 Click on Promo Selector
	And the user selects action 'Selector' and clicks: 'Payable Amount Selector'
	#9 In the promo selector Select the two promo created in preconditions 1 and 2, such as the sum of the OPA is < 0 and Click OK
	And the user opens the filter at column 'Action Code'
	And the user clicks on 'Filter Option' with value 'ActionCode1'
	And the user clicks on 'Filter Option' with value 'ActionCode2'
	And the user clicks on the 'OK Button'
	And the user clicks on element 'Popup Select Row Checkbox' with value 'Select Payable Amounts' and value '2'
	And the user clicks on element 'Popup Select Row Checkbox' with value 'Select Payable Amounts' and value '4'
	And the user clicks on element 'Popup Select Row Checkbox' with value 'Select Payable Amounts' and value '5'
	And the user clicks on the 'OK Button'
	#10 Check the columns from the selector grid
	#Selection check-box (test that the multiselection is allowed)
	And the user clicks on 'Popup All Checkbox By Name' with value 'Finalization'
	...
	And the user validates that the 'Grid CheckBox Containing' with value 'ActionCode2' is present
	#12 Check in the header of the popup if field 'Total Balance Amount' is present - > The field 'Total Balance Amount' is present and contains the sum of 'Amount to Balance' rows
	When the user retrieves text from element 'Input By Label Name' with value 'Total Balance Amount' and saves it as ''
    When the user retrieves text from 'Total Balance Amount Textbox' and saves it as 'TotalBalanceAmount'
