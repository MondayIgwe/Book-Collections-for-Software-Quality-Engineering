...
@TTC_Nightly @TCC @BDD_353834 @Pipeline
Scenario: WEB_SA_BALANCE_MATCHED_CLAIMS_PROMOTIONS_BALANCE_005
	...
	And the user populates the cockpit with description: '<description>' sellInStart: '<SELLINSTART>' sellInEnd: '<SELLINEND>' sellOutStart: '<SELLOUTSTARTDATE>' sellOutEnd: '<SELLOUTENDDATE>' mechanics: '<mechanic>'
	...
	And the user adds a product with productLevel: '<productLevel>' productCodes: '<product>'
	...
	And the user creates a Promo with customerHLevel: '<CUSTLEV>' customerCode: '<customerB>'
	And the user populates the cockpit with description: '<descriptionB>' sellInStart: '<SELLINSTART>' sellInEnd: '<SELLINEND>' sellOutStart: '<SELLOUTSTARTDATE>' sellOutEnd: '<SELLOUTENDDATE>' mechanics: '<mechanic>'
	And the user clicks on 'Side Panel Tab' with value 'General info'
	And the user clicks on 'Top Tab' with value 'Header info'
	And the user retrieves text from element 'Input By Label Name' with value 'Action code' and saves it as 'ActionCode2'
	And the user adds a product with productLevel: '<productLevel>' productCodes: '<Product2>'
	And the user adds a product with productLevel: '<productLevel>' productCodes: '<Product1>'
    ...
    And the user creates a New Claim with Document Type: '' Document Number: '<CustomerDocNr1>' Level: '<CUSTLEV>' Customer: '<customer>' Amount: '<amount>'
	...
	And the user filters the column by column name 'Payable Amount Logical Key', Filter Type 'Like', Value 'ActionCode1'
	...
	When the user navigates to the 'Claims' page under division'Claims'
    And the user creates a New Claim with Document Type: '' Document Number: '<CustomerDocNr1>' Level: '<CUSTLEV>' Customer: '<customerB>' Amount: '<amountB>'
	And the user clicks on 'Side Panel Tab' with value 'Summary'
	And the user retrieves text from element 'Input By Label Name' with value 'Document ID' and saves it as 'ClaimDocumentID2'
	And the user clicks on 'Side Panel Tab By Name' with value 'Matchings'
	And the user selects action 'Match To Generic Amounts' and clicks: 'Match To Payable Amounts'
	And the user opens the Grid Settings Menu option: 'Remove Filter' in the 'Select Payable Amount' pop up table
	And the user filters the column by column name 'Payable Amount Logical Key', Filter Type 'Like', Value 'ActionCode2'
	...
	Then the user validates that 'Popup Title' text contains '<FinalizationPopupTitle>'
	#7 Check the rows
	...
	And the user selects action 'Percentage' and clicks: '50%'
	...
	And the user validates that 'Tag Field By Label Name' with value 'Supplier' contains text '<supplier>'
