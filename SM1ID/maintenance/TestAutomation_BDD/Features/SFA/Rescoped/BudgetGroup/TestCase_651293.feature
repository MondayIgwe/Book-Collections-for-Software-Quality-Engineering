...
@SFA_Nightly @BDD_651293 @SFA_Weekly
Scenario: WEB_SFA_SF_BUDGET_018_BUDGET_WORKFLOW
	...
	And the user enters '<DefaultInitailValue>' in field 'Input By Label Name' with value 'Default initial value - PACKS'
	And the user press the 'TAB' key on the 'Input By Label Name' with value 'Default initial value - PACKS'
	...
	When the user clicks on 'Top Tab' with value 'History'
	...
	And the user filters the column by column name 'Code', Filter Type 'Like', Value 'BudgetCode'
	...
	And the user changes the status to 'Cancel'
	...
	And the user retrieves text from element 'Input By Label Name' with value 'Code' and saves it as 'BudgetCode2'
	...
	And the user filters the column by column name 'Code', Filter Type 'Like', Value 'BudgetCode2'
	When the user clicks on 'Indexed Row Link' with value '1'
	And the user clicks on 'Button' with value 'Edit'
	And the user clicks on 'Side Panel Tab' with value 'Workflow'
	And the user changes the status to 'Cancel'
	...
	And the user validates that the count for 'Text In Table Cell' with value 'Draft' is '2'
	#13 Go to Notes tab
	When the user clicks on 'Side Panel Tab' with value 'Note / Attachments'
	...
	Then the user validates that the 'Input By Label Name' with value 'Description' is present
	...
	Examples:
		| TypologyValue | OwnerLevelValue | BdgDescription1 | MeasureUnitValue | DefaultInitailValue | BdgDescription2 | MSG                      | StatusDescField | WkfStatusField  | LinkBudgetDetail | StatusCodeField | OwnerCode   | OwnerCode2   |
		| Free goods    | Sales rep       | BDG_018_1       | PACKS            | 1000                | BDG_018_2       | No available transitions | Status Desc.    | Workflow Status | Budget Detail    | Status Code     | AANASTASIEI | AANASTASIEI1 |