...
namespace Kantar_BDD.StepDefinitions
{
    ...
    public class GridStepDefinitions : SeleniumStepDefinition
    {
        ...
        {
            GridStepHelpers.ClickOnCheckboxCell(PopUpSectionGrid.SectionGridRows(gridLabel), PopUpSectionGrid.SectionGridColumnHeaders(gridLabel), dataColumnName, rowData, checkboxColumnName);
        }
        ...
        [When(@"the user enters '(.*)' in cell with Side Panel '(.*)' and column '(.*)' and row containing text '(.*)' in column '(.*)'")]
        public void WhenTheUserEntersValueInGridContaining(string value, string SidePanel, string columnName, string rowText, string rowTextColumnName)
        {
            value = GetValue(value);

            Selenium.Click(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowTextBySidePanel(SidePanel, columnName, rowText, rowTextColumnName));
            Selenium.ClearByKeys(SidePanelBasicGrid.InputBySidePanel(SidePanel));
            Selenium.SendKeys(SidePanelBasicGrid.InputBySidePanel(SidePanel), value);
        }
        ...
        public void WhenTheUserInPopUpRetrievesTextInGrid(string columnName, string rowText, string key)
        {
            ...
            toSave = Selenium.GetText(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText));
            Selenium.LooseFocusFromAnElement();
