...
Scenario: WEB_SA_AGREEMENT_AGREEMENT DOCUMENT_CONDITIONS_051
	...
	When the user creates a New Document with Level: '<CustomerLevel>' Customer: '<CustomerHolder>' Agreement Code: '' Agreement Description: ''
	...
	And the user populates the New Rebate Details Classification: '<Classification>' Reason: '<Reason>' Target Type: '<TargetType>' Payment Mode: '<PaymentMode>' Calculation Basis: 'Net sales revenue' Description: 'RebateFolder_051'
	...
	And the user selects validity period from: '01/02/y' to: '30/11/y' on the 'New Rebate Validity Period Trigger' field
	...
	And the user selects validity period from: '01/06/y' to: '30/11/y' on the 'Calc Basis Validity Period' field
	And the user selects validity period from: '01/01/y' to: '30/06/y' on the 'Target Validity Period' field
	# set dates
	...
	# Step 17
	When the user filters the column by column name 'Rebate code', Filter Type 'Not Like', Value 'MasterRebateCode'
	And the user retrieves text in cell with column 'Rebate code' and row text 'newDescription_051' and saves as 'ChildRebateCode'
	And the user filters the column by column name 'Rebate code', Filter Type 'Not Like', Value 'ChildRebateCode'
	...
	And the user filters the column by column name 'Rebate code', Filter Type 'Like', Value 'ChildRebateCode'
	...
	And the user filters the column by column name 'Rebate code', Filter Type 'Like', Value 'ChildRebateCode1'
	And the user clicks on link on column 'Rebate code' with row text 'newDescription_051'
	
	Then the user validates that the 'Input By Label Name' with value 'Rebate code' is displayed
	And the user validates that popup with sm1 ID 'CARD_RANGES' and name 'Benefit' column 'Threshold' has '<threshold4a>' with row text '<Percentage4>'
	...
	# Step 18
	When the user clicks on the 'Close Tab'
	And the user filters the column by column name 'Rebate code', Filter Type 'Like', Value 'MasterRebateCode'
	And the user clicks on link on column 'Rebate code' with row text 'newDescription_051'
