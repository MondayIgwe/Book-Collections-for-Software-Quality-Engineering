...
Scenario: TC_574078_WEB_SFA_MD_ASSO_036_ASSORTMENT_SIMULTION_FROM_CUSTOMER_UI
	...
	And the user filter the pop up menu: 'New Products: Selection 1/3' by column name 'Product', 'Like', '03'
	...
	And the user validates that the 'Div Text in Grid' with value '<ProductDescription1>' is displayed
	And the user validates that the 'Div Text in Grid' with value '<ProductDescription2>' is displayed
	...
	#Step 5
	When the user clicks on 'Trigger Open Dropdown' with value 'Customer Node'
	Then the user is on the '<Customer1>' page

	#Step 6
	When the user clicks on 'Side Panel Tab' with value 'Assortments'

	#Step 7
	Then 'Toolbar Grid Record Count' is populated with '0/2'
	...
	Then the user is on the '<CustomerDescription>' page
	
	#Test Clean up
	When the user filters the column by column name 'Assortment Description', 'Like', '<Description1>'
	And the user filters the column by column name 'Status', 'Like', 'Valid'
