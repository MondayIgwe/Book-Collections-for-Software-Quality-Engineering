...
Scenario: WEB_TPM_SP_PROMOPLANDOCUMENT_DRAGDROP_CREATENEW_PROMO_001
	...
	Then the user validates that the 'New Promo Action Popup' is present
	And the user validates the 'Confirmation Level Combo' is disabled
	When the user clicks on the 'OK Button'
	#The contractor is @PPHOLDER
	Then the user validates that the Loading Mask is not present
	And the user validates that 'Customer Holder Textbox' text contains '<PPHOLDER>'
	...
	When the user enters '<DESCR_NATIONAL2>' in the 'Promo Description Textbox'
