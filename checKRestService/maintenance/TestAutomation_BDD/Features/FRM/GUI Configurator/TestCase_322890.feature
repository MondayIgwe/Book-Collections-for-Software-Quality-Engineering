Feature: TestCase_322890

When the ALLOW NULL Property = TRUE set for Date Picker Field, then the Date Picker field is mandatory
When the ALLOW NULL Property = FALSE set for Date Picker Field, then the Date Picker field is optional

Scenario: WEB_FRM_CONFIGURATOR_DATE_PICKER_ALLOW_NULL
	...
	Then the user validates that the 'Date 2 Field Mandatory Icon' is displayed
    ...
    And the user validates that the 'Date 2 Field Mandatory Icon' is displayed
	...
	When the user clicks on the 'Date 2 Calendar Icon'
	When the user clears the text from 'Date 1 Field' element
	Then 'Toolbar Error Message Field' is populated with 'Date 2: Mandatory field empty'

	#Step9
    When the user selects 'Today' from calendar icon 'Date 2 Calendar Icon'

	#Step10 - 11 - 12
	When the user clicks on the 'Configurator Button'
	Then the user validates that the 'GUI Configurator On' is displayed
	When the user clicks on the 'Date 2 Field Settings Icon'
	Then the 'Gui Configurator Popup' is visible
    When the user clicks on the 'Allow Null Open Icon'
	Then the 'Value Field' is visible
	When the user clicks on the 'Value Dropdown Icon'
	...
	Then the user validates 'Date 2 Field Mandatory Icon' is not visible