...
Scenario: WEB_SA_AGREEMENT_REBATE DOCUMENT_GENERAL INFO_002
	...
	When the user creates a Rebate '<Level>' '<Customer>' '<Classification>' '<Reason>' '<Target>'
	...
	And the user populates the Rebate mandatory data '' '<Pay_Mode>' '<Pay_Freq>'
	When the user selects validity period '<StartDate>' '<EndDate>' on the 'Small Letter Rebate validity Period' field
	And the user enters '<Lumpsum>' in cell with column 'Fixed amount' and row containing text '0.0'
	And the user clicks on the 'Save Button'
	And the user clicks on the 'Close Tab'

	# Step 2
	And the user creates a Rebate '<Level>' '<Customer>' '<Classification>' '<Reason>' '<Target>'
    Then the user validates that the 'Input By Label Name' with value 'Rebate code' is displayed
    When the user retrieves text from element 'Input By Label Name' with value 'Rebate code' and saves it as 'RebateCode1'
	And the user populates the Rebate mandatory data '' '<Pay_Mode>' '<Pay_Freq>'
	When the user selects validity period '<StartDate1>' '<EndDate1>' on the 'Small Letter Rebate validity Period' field
	...
	# Step 7
	When the user selects validity period '<StartDate2>' '<EndDate2>' on the 'General Rebate Validity Period' field
	...
	# Step 14
	And the user filters the column by column name 'Agreement Code', 'Like', '<AGR_CODE>'
	And the user clicks on 'Text in Grid' with value 'AgreementID'
