...
Scenario: WEB_SFA_SE_VISIT_021_DETAIL_SECTION_IN_ACTIVITY_TAB_ON_CUSTOMER_ACTIVITIES_BDD
	...
	And the user filters the pop up menu with title: 'Columns Settings' by column name 'Caption' Filter condition: 'Like' Field Value: 'ID Visit'
	...
	And the user filters the column by column name 'ID Visit', Filter Type 'Like', Value '000000000000000210524111103764'
	And the user opens the grid item in row '1'
	Then the user validates that the user is on the 'Summary' page
	...
	And the user validates that 'HQ notes field' is populated with 'Test Note from HQ on detail section'
	And the user validates that 'HQ notes field' type is '<TextArea>'
	And the user validates that 'Preparation notes field' type is '<TextArea>'
	And the user validates that 'Notes field' is populated with 'V1 note on detail'
	And the user validates that 'Notes field' type is '<TextArea>'
	And the user validates that 'Last visit notes field' type is '<TextArea>'
	And the user validates that 'Time spent on activity label' is populated with 'Time spent on activity: 1 h 1 m'
	And the user validates that 'Time spent on activity label' type is 'label'
	...
	And the user filters the column by column name 'ID Visit', Filter Type 'Like', Value '000000000000000210524112427743'
	And the user opens the grid item in row '1'
	Then the user validates that the user is on the 'Summary' page
	
	When the user clicks on 'Side Panel Tab' with value 'Asset Store Check'
	And the user clicks on the 'Details Section Expand Panel button'
	Then the user validates that 'HQ notes field' type is '<TextArea>'
	And the user validates that 'Preparation notes field' is populated with 'V3 note on detail'
	And the user validates that 'Preparation notes field' type is '<TextArea>'
	And the user validates that 'Notes field' is populated with 'V4 note on detail'
	And the user validates that 'Notes field' type is '<TextArea>'
	...
	And the user validates that 'Last visit notes field' type is '<TextArea>'
	And the user validates that 'Time spent on activity label' is populated with 'Time spent on activity: 1 m'
	And the user validates that 'Time spent on activity label' type is 'label'
	And the user validates that 'Objective flag' is marked as (true/false): 'true'
	And the user validates that 'Objective flag' type is '<CheckBox>'
	...
	When the user closes all pages without saving the work done where there is no pop up
	And the user filters the column by column name 'ID Visit', Filter Type 'Like', Value '000000000000000210524112845826'
	And the user opens the grid item in row '1'
	Then the user validates that the user is on the 'Summary' page
	When the user clicks on 'Side Panel Tab' with value 'Asset Store Check'
	And the user clicks on the 'Details Section Expand Panel button'
	Then the user validates that 'HQ notes field' type is '<TextArea>'
	And the user validates that 'Preparation notes field' is populated with 'V4 note on detail'
	And the user validates that 'Preparation notes field' type is '<TextArea>'
	...
	And the user validates that 'Last visit notes field' type is '<TextArea>'
	And the user validates that 'Time spent on activity label' is populated with 'Time spent on activity: 2 m'
	And the user validates that 'Time spent on activity label' type is 'label'
