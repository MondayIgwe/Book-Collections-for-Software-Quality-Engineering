...
Scenario: WEB_TPM_IBP_PROCESS_CALENDAR_004-Process from Draft to Confirm to Cancelled Visible
	...
	And the user select value '<PROCESS_CODE>' from dropdown list 'Generic Drop Down Trigger'
	And the user clicks on 'Generic Drop Down Trigger' with value 'Reference Year'
	And the user select value '<REF_YEAR>' from dropdown list 'Generic Drop Down Trigger'
	...
	And the user select value '<STATUS>' from dropdown list 'Generic Drop Down Trigger'
	...
	And the user enters '<END_VALIDITY>' in field 'Input By Label Name' with value 'End Validity'
	And the user clicks on 'Generic Drop Down Trigger' with value 'Status'
	And the user select value '<STATUS1>' from dropdown list 'Generic Drop Down Trigger'
	...
	And the user enters '<END_VALIDITY>' in field 'Input By Label Name' with value 'End Validity'
	And the user clicks on 'Generic Drop Down Trigger' with value 'Status'
	And the user select value '<STATUS2>' from dropdown list 'Generic Drop Down Trigger'

	# Step 16 - Change the status of the Process selecting @STATUS3
	And the user clicks on 'Generic Drop Down Trigger' with value 'Status'
	And the user select value '<STATUS3>' from dropdown list 'Generic Drop Down Trigger'
