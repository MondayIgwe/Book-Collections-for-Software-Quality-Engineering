...
@BDD_335200 @PROMO_Nightly @FundsAndRules @FundsAndRulesNightly
Scenario: TPM_FR_W_SMOKE_R81V1_0006 - Movement Panel
	...
	Then the user validates that the 'Remaining Balance Textbox' is present
	And the user validates that the 'Uncommitted Balance Textbox' is present
	And the user validates that the 'Fund Reserve Textbox' is present
	...
	Then the user validates that the element 'Remaining Balance Textbox' has text '1,100.0000'
	And the user validates that the element 'Uncommitted Balance Textbox' has text '1,100.0000'
	And the user validates that the element 'Fund Reserve Textbox' has text '0.0000'
	...
	Then the user validates that the element 'Remaining Balance Textbox' has text '900.0000'
	And the user validates that the element 'Uncommitted Balance Textbox' has text '900.0000'
	And the user validates that the element 'Fund Reserve Textbox' has text '0.0000'
	...
	Then the user validates that the element 'Remaining Balance Textbox' has text '900.0000'
	And the user validates that the element 'Uncommitted Balance Textbox' has text '900.0000'
	And the user validates that the element 'Fund Reserve Textbox' has text '100.0000'
	...
	Then the user validates that the element 'Remaining Balance Textbox' has text '1,000.0000'
	And the user validates that the element 'Uncommitted Balance Textbox' has text '1,000.0000'
	And the user validates that the element 'Fund Reserve Textbox' has text '0.0000'
	#10 Take note of all data and movements that are in the panel and close the Movements Panel
	When the user retrieves count of 'All Rows' with value 'Movements' and saves it as 'RowCount'
	And the user clicks on the 'OK Button'
	#11 Open again Movements Panel -> All information previously shown are kept.
	And the user clicks on the 'Open Folder Icon'
	And the user retrieves count of 'All Rows' with value 'Movements' and saves it as 'RowCountNew'
