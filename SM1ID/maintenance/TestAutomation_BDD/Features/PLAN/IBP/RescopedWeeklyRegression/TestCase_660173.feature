Feature: TestCase_660173

@BDD_TC660173 @PLAN_Weekly @PLAN @Regression  @PLAN_Regression
Scenario: WEB_TPM_IBP_PROCESS_CALENDAR_005-Process from draft to confirm Hidden
	...
	# Step 12 -Switch to Edit and Change the status of the Process selecting @STATUS2
	...
	# Step 14 - The status of process @PROCESS_CODE, @REF_YEAR, @START_VALIDITY, @END_VALIDITY is @STATUS2
	And the user clicks on 'Button Acton link' with value containing 'refresh'
	And the user filters the column by column name 'Status', Filter Type 'Like', Value '<STATUS2>'
	Then the user validates that column 'Status' contains '<STATUS2>' with row text '<STATUS2>'

	# Step 15 - Open the IBP Calendar Navigator
	When the user navigates to the 'IBP Calendar' page

	# Step 16 - Check if the child calendar  @Description,@Type, @ChildProcess is in the Navigator
	# The status of @Description, @ProcessID, @Type, @ChildProcess is @STATUS3
	And the user clicks on 'Button Acton link' with value containing 'refresh'
	...
	| CBP2         | 2022     | 01/01/2022     | 01/01/2022   | Calendar7   | CBP2      | Hidden | Data Gather | Run Once  | Draft  | CONFIRM | Confirmed | Scheduled | Canceled | CBP2_2022    | AUTOM2   |
