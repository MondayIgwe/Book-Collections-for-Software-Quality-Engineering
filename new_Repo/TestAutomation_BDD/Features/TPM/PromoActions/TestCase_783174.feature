...
@BDD_783174 @PROMO_Nightly @noSellOutDate @PromoActions
Scenario: TPM_SP_W_SMOKE_R80V1_0016_NEW_BEHAVIOR
	...
	And the user populates the cockpit '<PROMODESC>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECHANICS>'
	...
	And the user clicks on 'Top Tab' with value 'Profit & Loss Retailer'
	...
	When the user clicks on 'Top Tab' with value 'Profit & Loss Retailer'
	...
	And user selects all products on Detail Data with unique row 'Total Promo Rebate' and column 'Promo Total / Current'
	...
	And the user clicks on 'Popup Ok Button' with value 'Selection'
	#open "Detail Data" combo and flag all checkboxes.
	And user selects all products on Detail Data with unique row 'Retail Net Cost' and column 'Action / Current'
	#Click on "Refresh" button 
	And the user clicks on the 'Profit And Loss Retailer Refresh Button'
