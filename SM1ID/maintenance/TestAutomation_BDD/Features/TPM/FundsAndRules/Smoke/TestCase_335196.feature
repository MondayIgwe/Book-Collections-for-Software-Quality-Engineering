Feature: TestCase_335196

@BDD_335196 @PROMO_Regression @PROMO_Smoke
Scenario: TPM_FR_W_SMOKE_R81V1_0004 - Create New F&R (Money Fund)
	#Step 1 - Go to "Fund & Rules" navigator
	Given the user navigates to the 'Funds & Rules' page
	#2 Create a New F&R
	When the user clicks on the 'Add Button'
	#3 Select "632 - Sales Fund" as @FR_CONFID1
	...
	And the user validates that the 'Input By Label Name' with value 'Configuration ID' is disabled
	...
	And the user validates that the column 'Fund value' is not editable
	...
	And the user validates that the 'Mandatory Icon' with value 'Fund value' is present
	And the user validates that the 'Input By Label Name' with value 'Fund value' is enabled
...
Examples:
	| FR_CONFID1       | Status | Description         | custLevel   | UnitMeasure | STARTSELLIN | ENDSELLIN |
	| 632 - Sales Fund | Draft  | Test F&R Sales Fund | 5 - Level 5 | Euro        | d/m/y       | d+1/m/y   |