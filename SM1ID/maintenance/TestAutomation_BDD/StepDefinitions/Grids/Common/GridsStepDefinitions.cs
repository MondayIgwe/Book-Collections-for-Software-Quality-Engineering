...
namespace Kantar_BDD.StepDefinitions
{
    ...
    public class GridStepDefinitions : SeleniumStepDefinition
    {
        ...
        {
            Selenium.Click(GridToolbarToDelete.DropDownMenu("Links"), 30);
            Selenium.Click(GridToolbarToDelete.DropDownMenuOption(linksPageNameOption));
        }
        ...
        public void WhenTheUserEntersValueInGrid(string value, string columnName, string rowText)
        {
            value = GetValue(value);
            Selenium.Click(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText));
            Selenium.ClearByKeys(BasicGrid.CellInputTextfield);
            Selenium.SendKeys(BasicGrid.CellInputTextfield, value);
            Selenium.LooseFocusFromAnElement();
        }
        ...
        {
            GridStepHelpers.ClickTriggerOnCell(BasicGrid.Columns, columnName, RebatesGrid.Rebatesrows, rowValue, "Rebates Grid Cell");
        }
        ...
        [When(@"the user filters the column by column sm1-id '([^']*)', Filter Type '([^']*)', Value '([^']*)'")]
        public void WhenTheUserFiltersTheColumnByColumnSm_IdFilterTypeValue(string columnSM1ID, string filterType, string value)
        {
            value = GetValue(value);
            StepHelpers.FilterGrid(columnName: columnSM1ID, filterType: filterType, filterValue: value, usingSM1ID: true);
        }

        [When(@"the user refreshes the grid with unique column '(.*)' until the column '(.*)' has '(.*)' in row number '(.*)'")]
        public void WhenTheUserRefreshesTheGridUntilTheColumnHasData(string uniqueColumn, string columnName, string value, string rowNumber)
        {
            int counter = 0;
            value = GetValue(value);
            ...
            AbstractedBy by = GridStepHelpers.GetAbstractedByFromTableWithUniqueColumn(uniqueColumn, columnName, Int32.Parse(rowNumber));
            string retrievedValue = Selenium.GetText(by);
            while (retrievedValue.CompareTo(value) < 0 && counter < 60)
            {
                Selenium.Click(GenericElementsPage.RefreshButtonGrid);
                Thread.Sleep(1000);
                counter++;
                retrievedValue = Selenium.GetText(by);
            }
            columnHasData = retrievedValue.CompareTo(value) >= 0;
            Assert.That(columnHasData, "Failed to validate that '" + columnName + "' has text '" + value + "' in row '" + rowNumber + "'.");
