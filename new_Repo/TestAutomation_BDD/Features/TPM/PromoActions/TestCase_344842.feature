...
@BDD_344842 @PROMO_Nightly @PromoActions
Scenario: TPM_SP_W_SMOKE_R80V1_0014 - Mechanics Selection
	...
	And the user populates the cockpit '<PROMODESC1>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTARTDATE>' '<SELLOUTENDDATE>' ''
	...
	And the user adds a product '<PRODUCT_LEVEL>' '<PROD1>'
	...
	And the user validates that column name '<MEAS_READONLY>' for product code '<PROD1>' 'is not' editable
	...
	When the user opens the trigger on product column 'Promotional Mechanics' and Row '<PROD1>'
	And the user clicks on 'PopUp Grid CheckBox' with value '<MECH_FUND_INJ>'
	...
	Then the user validates that column name '<MEAS_READONLY>' for product code '<PROD1>' 'is not' editable
	And the user validates that column name '<MECH_FUND_INJ>' for product code '<PROD1>' 'is' editable
	#8 Enable the deselected mechanic for another product [Def. Cut Price in %]
	When the user clicks on 'Side Panel Tab' with value 'General info'
	And the user clicks on 'Mechanics CheckBox' with value '<MECH3>'
	And the user clicks on the 'Alert Dialog No Button'
	And the user clicks on 'Side Panel Tab' with value 'Products'
	And the user opens the trigger on product column 'Promotional Mechanics' and Row '<PROD2>'
	And the user clicks on 'PopUp Grid CheckBox' with value '<MECH3>'
	And the user clicks on 'Popup Button' with value 'OK'
	...
	Then the user validates that column name '<MEAS_MECH3>' for product code '<PROD2>' 'is' editable
