...
@BDD_512567 @PROMO_Nightly @PromoActions
Scenario: WEB_TPM_SP_PRODSELECTOR_PROMOASSORTMENT_MANDATORY_AND_DISPLAY_001
	...
	And the user adds an assortment product '<PRODUCTS>'
	And the user adds an assortment product '<PRODUCTS2>'
	And the user adds an assortment product '<PRODUCTS3>'
	And the user adds an assortment product '<PRODUCTS4>'
	And the user adds an assortment product '<PRODUCTS5>'
	And the user adds an assortment product '<PRODUCTS6>'
	And the user adds an assortment product '<PRODUCTS7>'
	And the user adds an assortment product '<DISPLAY>'
	And the user adds an assortment product '<DISPLAY2>'
	And the user adds an assortment product '<DISPLAY3>'
	And the user adds an assortment product '<DISPLAY4>'
	...
	And the user creates a Promo '<CUSTOMERLEV>' '<CUSTOMER>'
	And the user populates the cockpit '<DESCR>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTARTDATE>' '<SELLOUTENDDATE>' '<MECHANICS >'
	...
	When the user clicks on 'Grid Checkbox' with value '<PRODUCTS>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS2>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS3>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS4>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS5>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS6>'
	And the user clicks on 'Grid Checkbox' with value '<PRODUCTS7>'
	...
	Examples:
		...
		| Promotion | Test Mandatory Assortment2 | EC86232  | 051      | 052       | 053       | 054       | 055       | 056       | 057       | 01/01/2100 | TRUE           | TPR in % | Holder level    | Level 3     | Promo having mandatory assortment | d+1/m/y     | d+15/m/y  |                  |                | TPR in %  | D001    | D002     | D003     | D004     |