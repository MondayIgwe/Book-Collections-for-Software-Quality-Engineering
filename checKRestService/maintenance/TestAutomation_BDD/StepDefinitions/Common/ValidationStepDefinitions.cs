...
namespace Kantar_BDD.StepDefinitions.Common
{
    ...
    public class ValidationgStepDefinitions : SeleniumStepDefinition
    {
        ...
        {
            value = GetValue(value);
            Assert.That(StepHelpers.ValidateElementVisible(Selenium.GetAbstractedBy(elementLogicalName, new String[] { value })), "Element " + elementLogicalName + " with value " + value + " did not qualify as being displayed");
        }
        ...
        public void ThenTheUserValidatesTheIsEmpty(string logicalName, string value)
        {
            ...
            Assert.IsTrue(StepHelpers.TextboxIsEmpty(Selenium.GetAbstractedBy(logicalName, new string[] { value })), logicalName + " with value '" + value + "' appears to have text " + Selenium.GetText(Selenium.GetAbstractedBy(logicalName, new string[] { value })));
        }

        [Then(@"the user validates the '([^']*)' is not empty")]
        public void ThenTheUserValidatesTheIsNotEmpty(string logicalName)
        {
            Assert.IsFalse(StepHelpers.TextboxIsEmpty(Selenium.GetAbstractedBy(logicalName)), logicalName + " appears to be empty, no data found in the value property");
        }
        ...
        {
            Assert.IsFalse(StepHelpers.TextboxIsEmpty(Selenium.GetAbstractedBy(logicalName, new String[] { value })), logicalName + " with value '" + value + "' appears to be empty, no data found in the value property");
        }

        [Then(@"the user validates the '([^']*)' is enabled")]
        public void ThenTheUserValidatesTheIsEnabled(string logicalName)
        {
            var element = Selenium.FindOne(Selenium.GetAbstractedBy(logicalName));
            Assert.That(element.Enabled, "Element " + logicalName + " did not qualify as being Enabled");
        }
        ...
        public void ThenTheUserValidatesTheIsDisabledWithParams(string logicalName, string value)
        {
            ...
            bool hasAttribute = Selenium.HasAttribute(Selenium.GetAbstractedBy(logicalName, values), "aria-readonly", "true") || Selenium.HasAttribute(Selenium.GetAbstractedBy(logicalName, values), "aria-disabled", "true") || Selenium.HasAttribute(Selenium.GetAbstractedBy(logicalName, values), "class", "disabled") || Selenium.HasAttribute(Selenium.GetAbstractedBy(logicalName, values), "disabled", "true") || Selenium.GetAttribute(Selenium.GetAbstractedBy(logicalName, values), "readonly").Equals("readonly") || Selenium.HasAttribute(Selenium.GetAbstractedBy(logicalName, values), "src", "disabled");
            if (!hasAttribute)
            {
        ...
        public void ThenTheWithValueIsNotDisplayed(string elementLogicalName, string value)
        {
            if (!value.Contains(";"))
            {
                AbstractedBy element = Selenium.GetAbstractedBy(elementLogicalName, SeleniumHelpers.CheckArguments(ScenarioContext, new string[] { value }));
                Assert.True(Selenium.FluentWaitElementToBeNotPresentInDOM(element) || Selenium.FluentWaitElementToBeNotVisible(element), $"Failed to wait for the element {elementLogicalName} with value {value} not to be displayed.");
                Assert.False(SeleniumHelpers.ValidateElementVisible(element), "Element " + elementLogicalName + " did not qualify as not being displayed");
            }
            else
            {
                string[] values = value.Split(";");
                ...
                {
                    AbstractedBy element = Selenium.GetAbstractedBy(elementLogicalName, SeleniumHelpers.CheckArguments(ScenarioContext, new string[] { val }));
                    Assert.True(Selenium.FluentWaitElementToBeNotPresentInDOM(element) || Selenium.FluentWaitElementToBeNotVisible(element), $"Failed to wait for the element {elementLogicalName} with value {value} not to be displayed.");
                    Assert.False(SeleniumHelpers.ValidateElementVisible(element), $"Element {elementLogicalName} with value {value} did not qualify as not being displayed");
                }
            }
        }
        ...
        {
            Selenium.FluentWaitElementToBePresent(Selenium.GetAbstractedBy(logicalName), 60);
            Assert.False(SeleniumHelpers.ValidateElementVisible(Selenium.GetAbstractedBy(logicalName)), $"Element {logicalName} did not qualify as not being displayed");
        }
        ...
        [Then(@"the user validates retrieved text is '(.*)'")]
        public void ThenTheUserValidatesRetrievedTextIs(string value)
        {
        ...
        {
            Assert.That(Selenium.IsAlertDisplayed()) ;
        }
        ...
        {
            Assert.True(StepHelpers.ValidateElementVisible(Selenium.GetAbstractedBy(elementLogicalName, SeleniumHelpers.CheckArguments(ScenarioContext, new String[] { value }, SeleniumHelpers.IsLogicalNameForColumnName(elementLogicalName)))), "Element " + elementLogicalName + " with value " + value + " did not qualify as visible");
        }
        ...
        {
            Assert.True(StepHelpers.ValidateGetDropdownOptions(dropdownOptions.Split(';').ToList()));
        }
        ...
        {
            Assert.IsTrue(Selenium.ValidateElementDisplayed(NavigationMenu.MenuButton, PageLoadTimeout), "Failed to login");
        }
    }
}