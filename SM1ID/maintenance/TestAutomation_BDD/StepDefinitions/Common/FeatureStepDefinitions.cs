...
namespace Kantar_BDD.StepDefinitions.Common
{
    ...
    public class FeatureStepDefinitions : SeleniumStepDefinition
    {
        ...
        {
            int counter = 0;
            while (!Selenium.ValidateElementDisplayed(TopToolbar.MenuButton, PageLoadTimeout) && counter < 180)
            {
                Thread.Sleep(1000);
                counter++;
            }
            Selenium.ClickUntilElementIsDisplayed(TopToolbar.MenuButton, NavigationMenu.ViewCompleteMenuButton);
            Selenium.Click(NavigationMenu.ViewCompleteMenuButton);
            Selenium.SendKeys(NavigationMenu.MenuSearchBar, page);
            Selenium.Click(NavigationMenu.SelectPage(division, page));
        }
        ...
        {
            Assert.IsTrue(Selenium.ValidateElementDisplayed(TopToolbar.MenuButton, PageLoadTimeout), "Failed to find the menu button for the navigate to page step");
            Selenium.ClickUntilElementIsDisplayed(TopToolbar.MenuButton, NavigationMenu.ViewCompleteMenuButton);
            Selenium.Click(NavigationMenu.ViewCompleteMenuButton);
            Selenium.SendKeys(NavigationMenu.MenuSearchBar, page);
            Selenium.ClickUntilElementIsDisplayed(NavigationMenu.SelectPage(page), TopToolbar.HomeToolbarPageName(page), 10);
        }
        ...
        public void GivenTheUserNavigateToThePageBySm1ID(string page, string sm1ID)
        {
            ...
            Selenium.Click(NavigationMenu.ViewCompleteMenuButton);
            Selenium.SendKeys(NavigationMenu.MenuSearchBar, page);
            Selenium.ClickUntilElementIsDisplayed(GenericElementsPage.ElementBySM1ID(sm1ID), TopToolbar.HomeToolbarPageName(page));
        }
        ...
        public void GivenTheUserHasLoggedInWithDivision(string division)
        {
            ...
            Selenium.ClickUntilElementIsDisplayed(UserProfile.UserCodeDropDownTrigger, GenericElementsPage.ListContaining(division));
            Selenium.ValidateElementDisplayed(GenericElementsPage.ListContaining(division), 15);
            Selenium.Click(GenericElementsPage.ListContaining(division), 15);

            Selenium.Click(GenericElementsPage.OkButton);
            if (Selenium.ValidateElementDisplayed(PopupGenericElements.AlertDialogYesButton))
                Selenium.Click(PopupGenericElements.AlertDialogYesButton);
        }

        [When(@"the user select value '([^']*)' from the combo '([^']*)'")]
        public void WhenTheUserSelectsComboValue(string valueToSelect, string fieldName)
        {
            if (valueToSelect.Equals("Retrieved"))
                valueToSelect = RetrievedValue;

            StepHelpers.SelectComboValue(valueToSelect, Selenium.GetAbstractedBy(fieldName));
        }

        [When(@"the user select value containing '([^']*)' from the combo '([^']*)'")]
        public void WhenTheUserSelectsComboValueContaining(string valueToSelect, string fieldName)
        {
            if (valueToSelect.Equals("Retrieved"))
                valueToSelect = RetrievedValue;

            StepHelpers.SelectComboValue(valueToSelect, Selenium.GetAbstractedBy(fieldName), false);
        }
        ...
        [When(@"the user retrieves text from '(.*)' and saves it as '(.*)'")]
        ...
        {
            Selenium.Click(SideTabs.WorkflowTab);
            Selenium.ClickUntilElementIsDisplayed(WorkflowPage.StatusTriggerIcon, GenericElementsPage.RadioButton(status));
        ...
        {
            Assert.IsTrue(StepHelpers.UploadFile(fileName, Selenium.GetAbstractedBy(logicalName, new string[] { fieldLabel })), "Failed to upload file '" + fileName +"'");
        }
        ...
        {
            Selenium.ValidateAllElementsLoaded(TopToolbar.BackButton);
            Selenium.Click(TopToolbar.BackButton, 30);
        ...
        public void WhenTheUserRunsCalculationSession(string liqCalc, string rebate)
        {
            ...
            Selenium.WaitForElementToBePresent(GuiToolbar.AddButton);
        ...
        public void WhenTheUserHoversOverWithValueUntilElementWithValueIsPresent(string elementName, string args, string elementToValidateName, string elementToValidateArgs)
        {
            ...
            {
                count++;
                Selenium.Hover(TopToolbar.KantarLogo);
                Thread.Sleep(1000);
                Selenium.Hover(Selenium.GetAbstractedBy(elementName, new string[] { args }));
                Thread.Sleep(1000);
            }
        ...
        public void WhenTheUserChecksCheckboxWithValue(string element, string elementValue)
        {
        ...
        public void WhenTheUserChecksCheckbox(string element)
        {
            if (!Selenium.ValidateCheckbox(Selenium.GetAbstractedBy(element)))
            {
                Selenium.Click(Selenium.GetAbstractedBy(element));
            }
        }
        ...
        [When(@"the user retrieves the system date in the format '([^']*)' and saves it as '([^']*)'")]
        public void WhenTheUserGetsTheSystemDateInTheFormat(string format, string variableName)
        {

            string date = System.DateTime.Now.ToString(format);
            SaveValue(variableName, date);
        }

        [When(@"the user clicks on the Action '([^']*)'")]
        public void WhenTheUserClicksOnTheActionButton(string ActionToClickElement)
        {
            Selenium.ValidateElementOnPage(GridToolbar.ActionsButton);
            int count = 0;
            Selenium.Click(GridToolbar.ActionsButton);

            while (!Selenium.FindOne(Selenium.GetAbstractedBy(ActionToClickElement)).Displayed && count < 5)
            {
                Thread.Sleep(500);
                Selenium.Click(GenericElementsPage.Button("Actions"));
                count++;
            }
            Selenium.Click(Selenium.GetAbstractedBy(ActionToClickElement));
        }
