...
@TTC_Nightly @TTC @BDD_424848
Scenario: WEB_SA_CLAIM_CLAIMS_MULTI_MATCH_009

	# Step 1
	Given the user navigates to the 'Claims' page under division'Claims'
	
    When the user creates a New Claim with Document Type: '' Document Number: 'WEB_SA_CLAIM_CLAIMS_MULTI_MATCH_009#1' Level: '<level>' Customer: '<customer>' Amount: '<amount1>'
	...
	# Step 2
	And the user creates a New Claim with Document Type: '' Document Number: 'WEB_SA_CLAIM_CLAIMS_MULTI_MATCH_009#2' Level: '<level>' Customer: '<customer>' Amount: '<amount2>'
	And the user clicks on 'Side Panel Tab' with value 'Summary'
	...
	And the user populates the cockpit with description: '' sellInStart: 'd-3/m/y' sellInEnd: 'd+3/m/y' sellOutStart: '' sellOutEnd: '' mechanics: '<mechanics>'
	...
	And the user adds products with productLevel: '<prodLevel>' productCodes: '<products>'
	...
	# Step 19
	And the user validates that the 'Finalization Grid' singlegrid contains the value '591.61' in the 'Amount to match' column in the row number '1'

	# Step20
	And the user validates that the 'Finalization Grid' singlegrid contains the value '546.16' in the 'Amount to match' column in the row number '2'
	...
	Then user validates that '1137.77' is a result of value 1: 'ClaimAmount1' operator: 'plus' value 2: 'ClaimAmount2'
