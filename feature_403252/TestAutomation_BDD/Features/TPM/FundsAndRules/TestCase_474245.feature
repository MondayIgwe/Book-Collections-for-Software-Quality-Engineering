...
@BDD_474245 @PROMO_Nightly @FundsAndRules
Scenario: WEB_TPM_SP_FRDOCUMENT_RATEBASEDFUND_CALCULATE_SOME_CONSTRAINTS_001
	...
	When the user creates a new fund and rules '<CONFIGURATIONID>' '<CUSTOMERLEVEL>' '<PAGEDESCRIPTION>' '<SELLINSTART>' '<SELLINEND>' '<PRODUCT_GROUPS>' '<RATEFUNDSOURCE>'
	And the user clicks on 'Side Panel Tab' with value 'Constraints'
	And user adds a '% Rate Fund' constraint with value '<INITIALRATEFUND1>' '<STARTDATE1>' '<ENDDATE1>' ''
	And the user clicks on 'Side Panel Tab' with value 'Summary'
	And the user clicks on 'Side Panel Tab' with value 'Constraints'
	And user adds a '% Rate Fund' constraint with value '<INITIALRATEFUND2>' '<STARTDATE2>' '<ENDDATE2>' ''
	And the user clicks on 'Side Panel Tab' with value 'Summary'
