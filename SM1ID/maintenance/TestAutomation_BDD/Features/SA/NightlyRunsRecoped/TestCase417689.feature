...
Scenario: WEB_SA_CLAIM_CLAIM DOCUMENT_MATCHINGS_003
    ...
    When the user creates a New Claim with Document Type: '' Document Number: '<Cust_417689>' Level: '<customerLevel>' Customer: '<customer>' Amount: '<Amount>'
	...
	And the user populates the cockpit with description: '' sellInStart: 'd/m/y' sellInEnd: 'd/m/y' sellOutStart: '' sellOutEnd: '' mechanics: '<mechanics>'
	...
	And the user adds products with productLevel: '<productLevel>' productCodes: '<product>'
	...
	# Step 8
	When the user filters the column by column name 'Action Code', Filter Type 'Like', Value 'ActionCode'
	...
	# Step 14
	And the user validates that the grid has the checkbox cell marked as 'false' in the 'Not Matched row' column with row value of '<promoMech2Code>'
