...
Scenario: WEB_SA_BALANCE_MATCHED CLAIMS_003
    ...
    And the user adds a product with productLevel: '<ProductLevel>' productCodes: '<ProductCode1>'
    And the user adds a product with productLevel: '<ProductLevel>' productCodes: '<ProductCode2>'
    ...
    And the user enters '<DocumentNumber>' in the 'Document Type Textbox'
    ...
    And the user enters '<Amount>' in the 'Amount Textbox'
    And the user clicks on the 'Document Type Textbox'
    ...
    Then the user validates that 'Input By Label Name' with value 'Status' has text 'Approved'
