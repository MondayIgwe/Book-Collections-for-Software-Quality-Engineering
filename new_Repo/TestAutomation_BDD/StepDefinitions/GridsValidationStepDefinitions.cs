...
namespace Kantar_BDD.StepDefinitions
{
    ...
    public class GridsValidationStepDefinitions : SeleniumStepDefinition
    {
        ...
        {
            Assert.True(GridStepHelpers.RetrieveCheckboxState(BasicGrid.RowNumberGridCheckbox(rowNumber.Trim())), $"Failed to validate that the checkbox at row number {rowNumber} is selected.");
        }
        ...
        public void ThenTheUserValidatesThatTheSingleGridHasTheCheckboxCellIsMarkedAsInTheColumnInTheRowWhereTheColumnHasValue(string trueOrFalse, string columnName, string rowData)
        {
            ...
            Assert.That(att.ToString().ToLower().Equals(trueOrFalse.ToLower()), $"Failed to validate that the checkbox cell is '{trueOrFalse}'. Expected: <{trueOrFalse}> Actual: <{att}>");
        }
    }
}
