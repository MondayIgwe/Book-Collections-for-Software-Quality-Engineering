...
@BDD_451221 @PROMO_Nightly @FundsAndRules
Scenario: WEB_TPM_SP_FR_QTYFUNDS_GRID
	...
	#Step 3 - 6
	And the user creates a new fund and rules '<CONFIGURATIONID>' '<CUSTOMERLEVEL>' '<PAGEDESCRIPTION>' '<STARTSELLIN>' '<ENDSELLIN>' '<PRODUCT_GROUPS>' '<RATEFUNDSOURCE>'
	...
	When user adds a 'Fund value' constraint with value '<FUNDVALUE>' '' '' ''
