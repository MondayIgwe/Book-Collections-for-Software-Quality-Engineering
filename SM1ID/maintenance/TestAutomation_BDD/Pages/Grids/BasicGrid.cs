...
namespace Kantar_BDD.Pages.Grids
{
    ...
    {
        public static AbstractedBy Grid = AbstractedBy.Xpath("Grid Filters","//div[@sm1-id='GRID_FR_FILTERS1']");
        ...
        public static AbstractedBy IndexedRow(string index) => AbstractedBy.Xpath("Indexed Row Link","(//table[contains(@id,'record')])["+index+ "]//span[@class='sm1-column-link']");
        public static AbstractedBy TextInGrid(string text) => AbstractedBy.Xpath("Text in Grid", "//*[text()='"+text+"']");
        public static AbstractedBy LinkInGrid(string text) => AbstractedBy.Xpath("Link In Grid", "//*[text()='" + text + "'][contains(@class,'link')]");
        public static AbstractedBy LinkContainerInGrid(string text) => AbstractedBy.Xpath("Link Container In Grid", "//*[text()='" + text + "'][contains(@class,'link')]/ancestor::td");
        public static AbstractedBy TextInGridContaining(string text) => AbstractedBy.Xpath("Text in Grid Containing", "//*[contains(text(),'" + text + "')]");
        public static AbstractedBy TextInTableCell(string text) => AbstractedBy.Xpath("Text In Table Cell", $"//table//tr//td//div[text()='{text}']");
        public static AbstractedBy ContainingTextInTableCell(string text) => AbstractedBy.Xpath("Containing Text In Table Cell", $"//table//tr//td//div[contains(text(),'{text}')]");

        public static AbstractedBy SelectIndexedRow(string index) => AbstractedBy.Xpath("Select Indexed Row","(//table[contains(@id,'record')]//tr//td[contains(@class,'checkcolumn')]//div[contains(@class,'checkcolumn')])[" + index + "]");
        public static readonly AbstractedBy GridSettings = AbstractedBy.Xpath("Grid Settings Menu", "//span[@sm1-id = 'GridContextMenuButton']");
        public static AbstractedBy GridSettingsOption(string option) => AbstractedBy.Xpath("Grid Settings Menu Option","//div[@sm1-id = 'GridContextMenu'][@aria-hidden = 'false']//a//span[@data-ref][text() = '" + option + "']");
        public static AbstractedBy SelectRow(string index) => AbstractedBy.Xpath("Select Row Checkbox","(//table//tr//td[contains(@class,'x-grid-cell-checkcolumn')]//div//span[@class='x-grid-checkcolumn'])["+ index + "]");
        public static AbstractedBy IterateCell(string rows, string rowNumber, string columnNumber) => AbstractedBy.Xpath("",rows + "[" + rowNumber + "]//tbody//tr//td[" + columnNumber + "]//div");
        public static AbstractedBy CellCheckboxImg(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div//img");
        public static AbstractedBy TableCell(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div");
        public static AbstractedBy CellInput(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div//input");
        public static AbstractedBy Cells(string row, string rowNum) => AbstractedBy.Xpath("",row + "[" + rowNum + "]//tbody//tr//td");
        public static AbstractedBy TextInCell(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div[text()]");

        public static AbstractedBy SpanTextInCell(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div//span[text()]");
        ...
        public static AbstractedBy LinkTextInCell(string rows, string row, string col) => AbstractedBy.Xpath("",rows + "[" + row + "]//tbody//tr//td[" + col + "]//div//a[text()]");
        ...
        public static AbstractedBy GridCheckBox(string uniqueRowText) => AbstractedBy.Xpath("Grid CheckBox", "(//*[text()='"+ uniqueRowText + "']/ancestor::tr[position()=1]//*[@class='x-grid-cell-inner x-grid-checkcolumn-cell-inner' or @src])[position()=1]");
        public static AbstractedBy GridCheckBoxContaining(string uniqueRowText) => AbstractedBy.Xpath("Grid CheckBox Containing", "//*[contains(text(),'"+ uniqueRowText + "')]/ancestor::tr[position() = 1]//*[@class='x-grid-cell-inner x-grid-checkcolumn-cell-inner' or (contains(@src,'chk') and not(contains(@src,'disabled')))]");
        public static AbstractedBy GridCheckBoxFromLinkDataContaining(string uniqueRowText) => AbstractedBy.Xpath("Grid CheckBox from link Containing", "//div//span[contains(text(),'" + uniqueRowText + "')]/ancestor::tr[position() = 1]//*[@class='x-grid-cell-inner x-grid-checkcolumn-cell-inner' or (contains(@src,'chk') and not(contains(@src,'disabled')))]");
        ...
        public static AbstractedBy ColumnContaining(string name) => AbstractedBy.Xpath("Basic Grid Column Containing", "//span[contains(@class,'x-column-header-text-inner') and contains(text(),'" + name + "')]");

        public static AbstractedBy CheckedGridCheckBoxContaining(string uniqueRowText) => AbstractedBy.Xpath("Checked Grid CheckBox Containing", "//div[contains(text(),'" + uniqueRowText + "')]/ancestor::tr[position() = 1]//td[@chkvalue='true']//*[@class='x-grid-cell-inner x-grid-checkcolumn-cell-inner' or (@src and not(contains(@src,'disabled')))]");
        ...
        public static AbstractedBy Cell(string row, string rowNum, string col) => AbstractedBy.Xpath("",row + "[" + rowNum + "]//tbody//tr//td[" + col + "]");
        ...
        public static AbstractedBy GridElipses(string uniqueRowText) => AbstractedBy.Xpath("Grid Elipses", "//div[text()='"+ uniqueRowText + "']/ancestor::tr//input[@type='button']");
        ...
        public static AbstractedBy NextPageButton = AbstractedBy.Xpath("Next Page Button", "//span[contains(@class,'x-tbar-page-next')]");
        public static AbstractedBy RowCells(string RowNumber) => AbstractedBy.Xpath("Row Cells", "(//table[contains(@id, 'tableview')][" + RowNumber + "]//div[@class='x-grid-cell-inner '  or @class='x-grid-cell-inner x-grid-cell-inner-action-col'])");
        public static AbstractedBy ColumnHeadings = AbstractedBy.Xpath("Column Headings", "//div[@sm1-id='GridContainer']//div[@role='columnheader' and @aria-hidden='false']//div[@class='x-column-header-text']//span[@data-ref]");
        //Gets the filter Icon by sm1-id where the column name cannot be used becasue there are multiple columns with the same name
        public static AbstractedBy FilterSM1ID(string columnsm1ID) => AbstractedBy.Xpath("Grid Filter sm1-id", "//div[@sm1-id='" + columnsm1ID + "'][@role='columnheader']//div[contains(@class,'filter')]");
        ...
        public static AbstractedBy columnFilter(string columnsm1ID) => AbstractedBy.Xpath("Column Filter", GenericElementsPage.columnFilterBySM1ID(columnsm1ID).ByToString);
        public static readonly AbstractedBy TotalNumberOfGridRows = AbstractedBy.Xpath("Total Number Of Grid Rows", "//div[@sm1-id='NavigatorGridToolbar']//div[@sm1-id='RowsNumLabel']");
    }
}