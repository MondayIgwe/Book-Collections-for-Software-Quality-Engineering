...
Scenario: WEB_SA_CLAIM_CLAIM DOCUMENT_MATCHINGS_032
	...
	And the user creates a Rebate with Level: '<CustomerLevel>' Customer: '<Customer>' Classification: '<classification>' Reason: '<reason>' Target Type: '<targettype>'
    Then the user validates that the 'Input By Label Name' with value 'Rebate code' is displayed
	...
	And the user populates the Rebate mandatory fields with Data Source: '<dataSource>' Payment Mode: '<paymentMode>' Payment Frequency: '<paymentFrequency>'
	And the user enters '<description2>' in field 'Input By Label Name' with value 'DESCRIPTION'
	And the user enters '<fixedAmount1>' in cell with column 'Fixed amount' and row containing text '0.0'
	And the user clicks on 'Side Panel Tab' with value 'General Info'
    And the user clicks on the 'Status Arrow Icon'
    And the user clicks on 'Radio Button' with value '<status>'
	...
	And the user creates a New Claim with Document Type: '' Document Number: 'CustomerDocument' Level: '<CustomerLevel>' Customer: '<Customer>' Amount: '<Amount>'
	...
	# Step 5
	And the user creates a New Claim with Document Type: '' Document Number: 'CustomerDocument2' Level: '<CustomerLevel>' Customer: '<Customer>' Amount: '<Amount>'
	...
	And the user validates that column 'Amount' has 'AmountToMatch3' with row text '<description>'
