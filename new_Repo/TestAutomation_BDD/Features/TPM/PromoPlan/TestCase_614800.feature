...
@BDD_614800 @PROMO_Nightly @applicationbug @PromoPlan
Scenario: WEB_TPM_SP_PROMOACTION_PREVENT_EDIT_NONRESPONSIBLE_USER_US592949_AC2_ON_004
	...
	When the user filters the column by column name 'Customer Code', 'Like', '<CUSTOMER>'
	...
	And the user assigns segments to user in responsibility matrix '<PRODDESC2>'
	...
	And the user clicks on 'Button' with value 'Cockpit'
	...
	And the user assigns segments to user in responsibility matrix '<PRODDESC1>'
	...
	When the user clicks on Calendar Promo Plan with value '<DESCPROMO>'
	#Click on 'Actions'
	#Click on 'Scenario re-assignment'
	And the user clicks on the Promo Action 'Scenario re-assignment'
