...
@BDD_428343 @PROMO_Nightly @PromoActions
Scenario: WEB_TPM_SP_PROMODOCUMENT_SINGLECOPY_001
	...
	#Step 2
	When the user creates a Promo '<HOLDERLEVEL>' '<HOLDER>'
	And the user populates the cockpit '<DESCRIPTION>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECH>'
	And the user adds a product '<PRODLEV>' '<PROD>'
	And the user adds a product '<PRODLEV>' '<PROD2>'
	...
	And the user clicks on 'Button' with value 'Products'
	...
	And the user clicks on 'Button' with value 'Cockpit'
	Then 'Description' is populated with '<NEWDESCRIPTIONVALUE>'
	...
	Then the user validates that the 'Button' with value 'Cockpit' is present
