...
@BDD_456215 @PROMO_Nightly @FundsAndRules @FundsAndRulesNightly
Scenario: WEB_TPM_SP_FRDOCUMENT_RATEBASEDFUND_CALCULATE_001
	...
	And user adds a ConstraintField: '% Rate Fund' constraint with value ConstraintValue: '<RATE_FUND>' startDate: '' endDate: '' familyConstraints: ''
	...
	And the user checks the 'Grid Row Checkbox' with value '<RATE_FUND>.00'
	Then the user validates that the 'Button Link' with value 'Calculate' is enabled
	#18 Select a Constraint @CONSTRAINT1 -> The "Calculate" button is enabled
	#When the user clicks on 'Text' with value '<RATE_FUND>.00'
	When the user unchecks the 'Grid Row Checkbox' with value '<RATE_FUND>.00'
	Then the user validates that the 'Button Link' with value 'Calculate' is disabled
	...
	Then the user validates that the 'Input By Label Name' with value 'Rate Fund Source' is disabled
	And the user validates that element: 'Button Link' with value: 'Edit' and attribute: 'aria-pressed' is in state: 'false'
