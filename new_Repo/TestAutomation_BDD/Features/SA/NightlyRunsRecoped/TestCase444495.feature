...
Scenario: WEB_SA_AGREEMENT_AGREEMENT DOCUMENT_CONDITIONS_004
	...
	# Step 6
	When the user clicks on 'Trigger By Field Name' with value 'Hier Filter'
	...
	# Step 14
	And the user validates that popup with sm1 ID 'LOGICALPRIZECUSTOMERFILTERMAKERPOPUP' and name 'Filter Maker' column 'ATTRIBUTE' has 'Code Node 1' with row text '<Customer_2>'
	...
	Then the user validates that the 'Popup' with value 'Save Filter (Required)' is not present

	# Step 17
	And 'Input By Label Name' text with value 'Hier Filter' contains '<Filter_Name_1>'
	...
	When the user in popup 'Filter Maker' enters '<Operator>' in cell with column 'Operator' and row text '<ATTRIBUTE>'
	...
	When the user clicks on 'Popup Ok Button' with value 'Save Filter (Required)'
	Then the user validates that the 'Popup' with value 'Save Filter (Required)' is not present
	And 'Input By Label Name' text with value 'Customer Filter' contains '<Filter_Name_2>'
