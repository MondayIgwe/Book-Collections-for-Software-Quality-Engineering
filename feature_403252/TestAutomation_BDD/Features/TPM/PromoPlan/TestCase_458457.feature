...
@BDD_458457 @PROMO_Nightly @PromoPlan
Scenario: WEB_TPM_SP_PROMOPLANDOCUMENT_FILTERPANEL_ACTIVESCENARIO_001
	...
	And the user creates a Promo '<PPHOLDERLEV>' '<PPHOLDER>'
	And the user populates the cockpit '<DESCR_NATIONAL>' '<SELLINSTART>' '<SELLINEND>' '<SELLOUTSTART>' '<SELLOUTEND>' '<MECHANICS>'
	And the user adds a product '<PRODLVL>' '<PROD1>'
	And the user adds a product '<PRODLVL>' '<PROD2>'
	...
	Then the 'Calendar Promo Plan' with value '<DESCR_NATIONAL>'  is not present
