...
Scenario: WEB_SA_AGREEMENT_AGREEMENT_DOCUMENT_CONDITIONS_097
	...
	And the user selects validity period from: '<StartDate>' to: '<EndDate>' on the 'Rebate Validity Period' field
	#3 Precondition3
	...
	And the user populates the Rebate Details Payment Mode: '<PaymentMode>' Data Source: '<CalculationBasis>' Description: '<RebateDescription2>'
	...
	When the user populates the cockpit with description: '<PromoDescription>' sellInStart: '<promoStartDate>' sellInEnd: '<promoEndDate>' sellOutStart: '<promoStartDate>' sellOutEnd: '<promoEndDate>' mechanics: '<Mechanic>'
	...
	When the user adds a product with productLevel: '<ProductLevel>' productCodes: '<ProductCode>'
	...
	When the user selects validity period from: '<StartDate>' to: '<RebateUpdateEndDate>' on the 'Rebate Validity Period' field
	...
	And the user filters the column by column name 'Rebate code', Filter Type 'Like', Value 'RebateCode2'
	And the user opens the grid item in row '1'
	And the user clicks on 'Button' with value 'Edit'
	#15 Change the validity: @StartDay @StartMonth Current year- @EndDay3 @EndMonth4 Current year
	When the user selects validity period from: '<StartDate>' to: '<RebateUpdateEndDate>' on the 'Small Letter Rebate validity Period' field
	And the user clicks on the 'Save Button'
	When the user clicks on the 'Close Tab'
