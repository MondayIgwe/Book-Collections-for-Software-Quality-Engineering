...
@TTC_Nightly @TTC @BDD_454642 @Pipeline
Scenario: WEB_SA_BALANCE_MATCHED CLAIMS_BALANCE_002
	
	# Step 1
	Given the user navigates to the 'Promotional actions' page

	When the user creates a Promo with customerHLevel: '<CustomerLevel>' customerCode: '<customer>'
	And the user populates the cockpit with description: '<description1>' sellInStart: '<SellInStart>' sellInEnd: '<SellInEnd>' sellOutStart: '' sellOutEnd: '' mechanics: '<mechanic>'
	And the user adds products with productLevel: '<prodLevel>' productCodes: '<product>'
	...
	And the user creates a Promo with customerHLevel: '<CustomerLevel2>' customerCode: '<customerB>'
	And the user populates the cockpit with description: '<description2>' sellInStart: '<SellInStart>' sellInEnd: '<SellInEnd>' sellOutStart: '' sellOutEnd: '' mechanics: '<mechanicB>'
	And the user adds products with productLevel: '<prodLevel>' productCodes: '<productB>'
	...
	And the user creates a New Claim with Document Type: '' Document Number: 'test case 454642' Level: '<CustomerLevel2>' Customer: '<customerB>' Amount: '<amount>'
	...
	When the user filters the column by column name 'Action Code', Filter Type 'Like', Value 'ActionCode1'
	...
	And the user in popup 'Matching finalization' Clicks cell with column 'Last Claim' and row text '<description1>'
	...
	# Step 10
    When the user clicks on 'Popup Ok Button' with value 'Finalization'
	...
	# Step 17
	When the user clicks on the 'Close Tab'
	And the user navigates to the 'Balances' page under division'Claims'
