...
Scenario: TC_506646_WEB_SFA_MD_ASSO_018_ASSORTMENT_SIMULATION
	...
	Then the user validates that the 'Grid Name' with value 'GridContainer' is present
	...
	Then the user validates that the 'Label Name' with value 'Hier. level' is present
	Then the user validates that the 'Label Name' with value 'Level' is present
	Then the user validates that the 'Label Name' with value 'Level' is present
	Then the user validates that the customer '<Customer1>' on row '1' is present
	...
	When the user creates a new assortment of Type: '<Type1>', Assortment Type: '<AssortmentType>', Customer Level: '<CustomerLevel1>', Customer: '<CustomerNode1>', Start Date: 'd/m/y', End Date: 'd/m/y', Description: 'ASSO_TEST_506646'
	And the user clicks on 'Side Panel Tab' with value 'Customers'
	Then the user validates that the 'Customer Simulation' is present
	Then the user validates that the 'Grid Name' with value 'GridContainer' is present
	Then the user validates that the customer '<Customer1>' on row '1' is present
