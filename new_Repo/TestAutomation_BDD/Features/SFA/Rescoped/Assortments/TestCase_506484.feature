...
Scenario: TC_506484_WEB_SFA_MD_ASSO_015_OVERLAPPING_ASSORTMENTS_UI_VALIDATIONS
	...
	When the user creates a new assortment of Type: '<AssortmentType>', Customer Level: '<CustomerLevel>', Customer: '<Customer1>', Start Date: '<StartDate2>', End Date: '<EndDate2>', Description: '<Description2>'
	And the user selects validity period '<StartDate1>' '<EndDate2>' on the 'Generic Drop Down Trigger' element with value 'Validity Period'
	And the user clicks on 'Button' with value 'Save'
	...
	And the user is on the 'Assortments' page
