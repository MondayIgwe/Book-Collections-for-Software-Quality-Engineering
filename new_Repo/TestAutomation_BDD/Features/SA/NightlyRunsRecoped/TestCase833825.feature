...
Scenario: WEB_SA_CLAIM_CLAIM DOCUMENT_047_AC_1.1
	...
	And the user creates a New Claim '<CustomerDocumentNr>' '<CustomerLevel>' '<CustomerHolder>' '<Amount>'
    ...
    Then the user validates that the 'Input By Label Name' with value 'VAT Code' is present
	...
	And the user enters '<Amount>' in field 'Input By Label Name' with value 'Amount'
