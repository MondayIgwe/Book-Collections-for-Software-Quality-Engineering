...
@BDD_573258 @bug @PromoActions @PromoActionsNightly
Scenario: WEB_TPM_SP_PROMOPLANDOCUMENT_ACTIONS_REMOVESCENARIO_001
	...
	And the user retrieves text from 'Action ID Textbox' and saves it as 'Promo1_ID'
	...
	And the user enters '<DESCR_2>' in the 'Promo Description Textbox'
	And the user clicks on the 'Save Button'
	...
	#10 Get $Promo2_ID
	And the user retrieves text from 'Action ID Textbox' and saves it as 'Promo2_ID'
	...
	And the user clicks on the Action 'Remove Existing Scenario'
	And the user select value 'Scenario to be deleted' from the combo 'Selected Scenario Remove Combo'
