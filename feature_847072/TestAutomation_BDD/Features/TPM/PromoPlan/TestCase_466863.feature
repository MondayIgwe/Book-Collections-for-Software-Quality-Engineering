...
Scenario: WEB_TPM_SP_PROMOPLANDOCUMENT_SOURCE_SCENARIO_FIELD_POSSIBLE_LOV_RESTRICT_US434914_AC1_002
	...
	And the user retrieves text from element 'Generic Drop Down Selected Value' with value 'Source Scenario' and saves it as 'SourceScenario'
	...
	When the user select value containing '<Value>' from dropdown list 'Source Scenario'
	And the user select value containing '<Scenario_Description_AltScen1>' from dropdown list 'Source Scenario'
	And the user select value containing 'SourceScenario' from dropdown list 'Source Scenario'
