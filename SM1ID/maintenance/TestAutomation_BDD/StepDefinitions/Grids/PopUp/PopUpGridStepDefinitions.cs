...
namespace Kantar_BDD.StepDefinitions
{
    ...
    public class PopUpGridStepDefinitions : SeleniumStepDefinition
    {
        ...
        public void WhenTheUserClicksOnPopUpTableWhereColumnHasTheValueAndTheCheckboxIsInColumn(string TableName, string DataColumn, string RowData, string CheckboxColumn, string onOrOff)
        {
            ...
            Thread.Sleep(250);
            GridStepHelpers.ClickOnCheckboxImgCell(GridPopups.Rows(TableName), GridPopups.PopUpColumns(TableName), DataColumn, RowData, CheckboxColumn, on);
        }

        [When(@"the user filters the pop up menu with title: '([^']*)' by column name '([^']*)' Filter condition: '([^']*)' Field Value: '([^']*)'")]
        ...
        {
            StepHelpers.FilterGrid(columnName, fieldName, fieldValue, popUpName);
        }


        [When(@"the user filters the pop up menu with title: '([^']*)' by column name '([^']*)' for the checkbox state: '([^']*)'")]
        ...
        {
            StepHelpers.FilterGridOnCheckboxColumn(popUpName, columnName, checkboxToClick);
        }

        [When(@"the user filters the pop up menu with title: '([^']*)' by column name '([^']*)' field name: '([^']*)' and retieved value")]
        public void WhenTheUserFiltersThePopUpMenuByColumnNameRetievedValue(string popUpName, string columnName, string fieldName)
        {
            StepHelpers.FilterGrid(columnName, fieldName, RetrievedValue, popUpName);
        }
        ...
        public void WhenTheUserSelectsRowOnThePopUpTable(string rowNumber, string popUpName)
        {
            ...
            Selenium.ValidateElementDisplayed(GridPopups.SelectRowCheckbox(popUpName, rowNumber), 30);
        ...
        public void TheTheUserValidatesThatGridColumnValue(string popupName, string columnName, string value, string rowText)
        {
            ...
            string retrievedValue = Selenium.GetText(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText, popupName));
            Assert.AreEqual(value, retrievedValue, "Failed to validate that that pop up '" + popupName + "' with column name '" + columnName + "' has '" + value + "' where row text is '" + rowText + "'");
        }
        ...
        public void TheTheUserValidatesThatGridColumnValueWithSMONEID(string smOneId, string popupName, string columnName, string value, string rowText)
        {
            value = GetValue(value);
            rowText = GetValue(rowText);
            string retrievedValue = Selenium.GetText(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText, popupName, smOneId));
            Assert.AreEqual(value, retrievedValue.Trim(), "Failed to validate that that pop up '" + popupName + "' with column name '" + columnName + "' has '" + value + "' where row text is '" + rowText + "'");
        }
        ...
        public void TheTheUserValidatesThatGridColumnContainsValueWithSMONEID(string smOneId, string popupName, string columnName, string value, string rowText)
        {
            ...
            rowText = GetValue(rowText);
            string retrievedValue = Selenium.GetText(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText, popupName, smOneId));
            Assert.That(retrievedValue.Trim().Contains(value), "Failed to validate that that pop up '" + popupName + "' with column name '" + columnName + "' contains '" + value + "' where row text is '" + rowText + "'");
        }
        ...
        public void WhenTheUserEntersValueInGridContainingInSpecificGrid(string value, string columnName, string rowText, string popupName)
        {
            ...
            try
            {
                Selenium.SendKeys(GridPopups.PopupGridCellInput(popupName), value);
            }
        ...
        {
            rowText = GetValue(rowText);
            string retrievedValue = Selenium.GetText(GridStepHelpers.GetGridAbstractedByByColumnNameAndRowText(columnName, rowText, popup));
